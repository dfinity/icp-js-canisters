...
/**
 * An asset a deposit pipeline event names: ETH, or an ERC-20 token by its contract address
 * in EIP-55 format.
 */
export type Asset = { Eth: null } | { Erc20: string };
/**
 ...
 */
export interface AuthorizedSweepItem {
  owner: Principal;
  subaccount: [] | [Uint8Array];
  /**
 ...
 */
export interface DepositEthArg {
  mode: DepositMode;
}
export type DepositEthError =
  | {
      /**
       * The minter is temporarily unavailable, retry the request.
       */
 ...
 */
export interface DepositEthResponse {
  /**
   * Where the deposit stands in the detect-and-sweep pipeline.
   */
  status: DepositEthStatus;
  /**
 ...
 */
export interface DetectedEthDeposit {
  /**
   * The Ethereum block at which the balance was detected.
   */
  detected_at_block: bigint;
...
export interface Event {
  ...
  payload:
    ...
    | {
        /**
         * A ckERC20 deposit address attested to the account it credits. Recorded
         * on its own because signing costs a threshold-ECDSA signature: the
         * attestation outlives the sweep that needed it and every later one
         * reuses it.
         */
        AttestedDepositAddress: {
          ...
          attestation: TransactionSignature;
          /**
           * The deposit helper the attestation names; it is only valid against
           * this deployment.
           */
    ...
    | {
        AcceptedSweepRequest: {
           ...
           */
          items: Array<AuthorizedSweepItem>;
          sweep_id: bigint;
        };
      }
    | {
...
export interface InitArg {
   ...
   */
  next_sweeper_transaction_nonce: [] | [bigint];
  /**
...
export interface MinterInfo {
   ...
   */
  minimum_eth_deposit_amount: [] | [bigint];
  /**
   * Address of the sweeper smart contract.
   */
  ...
  /**
   * Dedicated Ethereum address controlled by the minter via threshold ECDSA, whose ETH
   * balance is the prepaid gas available for sweeping funded deposit addresses.
   */
 ...
 */
export interface UnsignedSweeperTransaction {
  transaction: UnsignedTransaction;
...
export interface _SERVICE {
  ...
  >;
  /**
   * Derive and register the ETH deposit address for the caller.
   * The account is derived from the caller's principal and the subaccount in the argument.
   * Returns the EIP-55 checksummed deposit address, which is the same address as the one
   * derived by deposit_erc20 for the same account, whatever the ERC-20 token, together with
   * `valid_until`, the timestamp (nanoseconds since the Unix epoch) until which a deposit to
   * it is guaranteed to be noticed, and the minimum balance in wei the address must hold to
   * be noticed.
   */
  deposit_eth: ActorMethod<
    [DepositEthArg],
    { Ok: DepositEthResponse } | { Err: DepositEthError }
  >;
  /**
